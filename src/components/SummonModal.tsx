...
export const SummonModal: React.FC<SummonModalProps> = ({ 
  ...
  const { summonBlob: executeSummonBlob } = useBlobs();
  ...
  const handleInitiateSummon = async () => {
    if (cubes < 1500 || isSummoning) return;
    setIsSummoning(true);
    playTapSound();
    setStage('signing');
    ...
    if (provider && rawWalletAddress) {
      try {
        ...
        const result = await executeSummonBlob();
        ...
        if (txHash) {
          ...
          if (updateState) {
            ...
            if (triggerToast) {
              setTimeout(() => {
                triggerToast('🎉 Base Builder Code detected! On-chain gas rewards & rebates successfully attributed to the App Developer.');
              }, 1000);
            }
          }
        }
          ...
          {stage === 'prepare' && (
            ...
            >
              ...
              <div className="w-full space-y-2">
                <motion.button
                  whileHover={{ scale: isSummoning ? 1.0 : 1.02 }}
                  whileTap={{ scale: isSummoning ? 1.0 : 0.98 }}
                  onClick={handleInitiateSummon}
                  disabled={cubes < 1500 || isSummoning}
                  className={`w-full py-3.5 rounded-2xl text-xs font-black text-white bg-gradient-to-r from-[#0052ff] to-[#00cfff] border border-white/10 hover:brightness-110 active:scale-95 transition-all shadow-lg shadow-blue-500/20 cursor-pointer ${
                    (cubes < 1500 || isSummoning) ? 'opacity-50 cursor-not-allowed filter grayscale' : ''
                  }`}
                >
                  {isSummoning ? 'PREPARE IN WALLET...' : 'SIGN & INITIATE CONTRACT'}
                </motion.button>
                <button
                  onClick={onClose}
