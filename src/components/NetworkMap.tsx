...
function NodePanel({ node, onClose, onAttack, onCollect, pendingCubes, walletAddress }: NodePanelProps) {
...
interface NetworkMapProps {
  ...
  collectFromNode: (node: import('../types').NetworkNode) => Promise<{ earned: number; error: string | null }>;
  collectAll: () => Promise<{ earned: number; error: string | null }>;
...
export function NetworkMap({
  ...
  // Границы пана: дальше самой внешней ноды тащить карту некуда
  const panLimits = React.useMemo(() => {
    ...
    // Оставляем пол-вьюпорта воздуха, чтобы крайние ноды можно было вывести в центр
    return {
      maxX: Math.max(0, maxAbsX + HEX_SIZE - VP_W / 2),
      maxY: Math.max(0, maxAbsY + HEX_SIZE - VP_H / 2),
    };
  }, [nodes]);

  // Drag handlers.
  // Живут только на <svg> — у нод НЕТ своего onPointerDown со stopPropagation,
  // иначе пан, начатый пальцем с ноды, не стартовал вообще (а ноды занимают
  // почти всю площадь карты — отсюда и «карта не двигается» на телефоне).
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    const DRAG_THRESHOLD = 6; // ниже порога считаем жест тапом, а не паном
    if (!hasDraggedRef.current && (Math.abs(rawDx) > DRAG_THRESHOLD || Math.abs(rawDy) > DRAG_THRESHOLD)) {
      hasDraggedRef.current = true;
      // Захват ставим только когда жест точно стал паном: иначе он бы перецелил
      // click на <svg> и тап по ноде перестал бы открывать панель
      try {
    ...
    if (hasDraggedRef.current) {
      ...
      // Перевод пикселей экрана в единицы viewBox уже даёт движение 1:1 с пальцем.
      // Прежний множитель 2.0 гнал карту вдвое быстрее жеста — отсюда «криво работает».
      const clamp = (v: number, limit: number) => Math.max(-limit, Math.min(limit, v));

      setOffset({
        x: clamp(dragStart.current.ox + rawDx * scaleX, panLimits.maxX),
        y: clamp(dragStart.current.oy + rawDy * scaleY, panLimits.maxY),
      });
    }
  };
  ...
  const handleCollect = async (node: NetworkNode) => {
    const { earned, error } = await collectFromNode(node);
    if (earned > 0) {
      onToast?.(`💠 +${earned} collected!`);
    } else if (error) {
      onToast?.(`❌ ${error}`);
    }
    setSelectedNode(null);
  ...
  const handleCollectAll = async () => {
    const { earned, error } = await collectAll();
    if (earned > 0) {
      onToast?.(`💠 +${earned} collected!`);
    } else if (error) {
      onToast?.(`❌ ${error}`);
