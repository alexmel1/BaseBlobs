              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 mt-1 pt-1.5 border-t border-white/5">
                  ...
                  {isOwned ? (
                    ...
                    <button
                      onClick={() => onUnlockSpecies(pk)}
                      disabled={cubes < 3000}
                      className={`py-2 rounded-xl text-[10px] font-extrabold flex items-center justify-center gap-1 transition-all ${
                        cubes >= 3000
                          ? 'bg-gradient-to-r from-blue-600 to-cyan-500 text-white hover:brightness-110 active:scale-95 cursor-pointer shadow-lg shadow-blue-500/10'
                          : 'bg-white/5 border border-white/5 text-slate-500 cursor-not-allowed'
                      }`}
                    >
                      <Sparkles className="w-3 h-3 text-[#ffc600]" />
                      <span>Unlock (3000 💠)</span>
                    </button>
                  )}
                </div>
              </div>
