...
interface WalletModalProps {
  ...
  onConnect: (walletType: 'base', address?: string) => void;
  onDisconnect: () => void;
  triggerToast: (msg: string) => void;
  initialConnectType?: 'base' | null;
}
...
export const WalletModal: React.FC<WalletModalProps> = ({
  ...
  const [connectingType, setConnectingType] = useState<'base' | null>(null);
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      setProgress(0);
      setErrorMsg(null);
    } else if (initialConnectType) {
      handleStartConnect(initialConnectType);
    }
  }, [isOpen, initialConnectType]);

  if (!isOpen) return null;

  const handleStartConnect = async (type: 'base') => {
    setErrorMsg(null);
    ...
    if (typeof window !== 'undefined') {
      ...
      // Exclusively check for Base/Coinbase provider, do not fall back to MetaMask
      ...
      // If no injected provider is found (e.g. desktop Chrome), use CoinbaseWalletSDK
      if (!provider) {
        provider = getCoinbaseSDKProvider();
      }
    }

    if (provider) {
      setStep(2); // Go to connecting step
      setProgress(20);
      try {
        setProgress(50);
        ...
        if (accounts && accounts[0]) {
          ...
          const formattedAddr = rawAddr.slice(0, 6) + '...' + rawAddr.slice(-4);
          setConnectedAddr(formattedAddr);
          
          setTimeout(() => {
            setProgress(100);
            onConnect(type, rawAddr); // pass raw address
            setStep(3); // success
          }, 600);
        } else {
        ...
        setErrorMsg(e?.message || 'Connection request rejected or closed.');
        setStep(4);
        setProgress(0);
      }
    } else {
        ...
        <div className="p-4 flex-1">
          {isWalletConnected && step !== 3 ? (
            ...
            <div className="flex flex-col gap-3 animate-fade-in">
              <p className="text-slate-400 text-[9.5px] leading-relaxed mb-1">
                Connect your wallet to anchor your progress to your blockchain Web3 identity.
              </p>

              {/* Base App option */}
              <button
                onClick={() => handleStartConnect('base')}
                className="w-full flex items-center justify-between p-3.5 bg-[#00a6ff]/10 hover:bg-[#00a6ff]/20 border border-[#00a6ff]/35 rounded-xl transition-all group cursor-pointer text-left relative overflow-hidden"
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-white text-xs font-bold">Base App</span>
                      <span className="bg-cyan-500/20 text-[#00cfff] border border-cyan-500/30 text-[8px] font-mono font-bold px-1 py-0.2 rounded flex items-center gap-0.5 animate-pulse">
                        <Zap className="w-2 h-2" /> Gasless
                      </span>
                    </div>
                    <span className="text-slate-400 text-[9px] block mt-0.5 font-sans">Connect within Base App & Browser</span>
                  </div>
                </div>
