...
import { useArena, ARENA_REWARDS } from './hooks/useArena';
...
export default function App() {
  ...
  const [state, setState] = useState<GameState>(() => {
      ...
      if (saved) {
        ...
        if (parsed && parsed.blobs && parsed.blobs.length > 0) {
          ...
          return {
            ...
            blobCharms: parsed.blobCharms ?? 0,
            lastExpeditionEvent: parsed.lastExpeditionEvent ?? null,
            lastFusionTime: parsed.lastFusionTime ?? 0,
            totalCubesAllTime: parsed.totalCubesAllTime ?? 0,
            totalExpeditionsAllTime: parsed.totalExpeditionsAllTime ?? 0,
            hasOGBadge: parsed.hasOGBadge ?? false,
            ...
            lastArenaRewardClaimed: parsed.lastArenaRewardClaimed ?? false,
            initialized: false,
          };
        }
      }
    ...
    // Default State
    return { ...getDefaultState(), initialized: false };
  });
  ...
  const leaderboard = useLeaderboard(
    rawWalletAddress,
    state.playerName ?? 'Trainer',
    state.blobs,
    state.totalExpeditionsAllTime ?? 0,
  );

  const registeredBlob = state.blobs.find(b => b.id === state.arenaRegisteredBlobId) || null;
  const arenaScore = leaderboard.myScore + (registeredBlob ? calcBlobPower(registeredBlob) * 2 : 0);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      updateState((prev) => {
        ...
        // Mood decay — drops by 1 every 12 hours without feeding
        let moodChanged = false;
        const updatedBlobs = prev.blobs.map(b => {
          const mood = b.mood ?? { level: 2, lastFed: Date.now(), winsToday: 0, lossesToday: 0 };
          const hoursSinceFed = (Date.now() - (mood.lastFed ?? Date.now())) / 3600000;
          if (hoursSinceFed >= 12 && mood.level > 0) {
            moodChanged = true;
            return {
              ...b,
              mood: {
                ...mood,
                level: Math.max(0, mood.level - 1) as import('./types').MoodLevel,
                lastFed: Date.now(),
              },
            };
          }
        ...
        if (prev.activeExpeditions.length > 0) {
          ...
          for (const exp of currentActiveExps) {
            if (Date.now() >= exp.endTime) {
              ...
              // Roll expedition event
              const event = rollExpeditionEvent();
              reward = Math.round(reward * event.cubeMultiplier);
              xpGain = Math.round(xpGain * event.xpMultiplier);

              // Blob Charm — doubles cubes
              if ((exp as any).charmActive) {
                reward = Math.round(reward * 2);
              }

              // Bonus item
              if (event.bonusItem === 'blob_charm') {
                prev.blobCharms = (prev.blobCharms ?? 0) + 1;
              }

              // Save event for display
              prev.lastExpeditionEvent = event;

              // Show event toast (replace the standard toast below if event is not 'normal')
              if (event.type !== 'normal') {
                triggerToast(`${event.icon} ${event.title}! ${event.cubeMultiplier > 1 ? `+${Math.round((event.cubeMultiplier-1)*100)}% cubes` : event.cubeMultiplier < 1 ? `-${Math.round((1-event.cubeMultiplier)*100)}% cubes` : event.bonusItem ? 'Got a Blob Charm!' : ''}`);
              }
              ...
              if (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'granted') {
                try {
                  new Notification(`Expedition Finished! 🎉`, {
                    body: `Your expedition to "${exp.name}" is complete! Earned: +${reward} 💠 and +${xpGain} XP!`,
                    tag: `expedition-${(exp as any).id || exp.zoneId}`,
                  });
                } catch (err) {
  ...
  // Feeding the blob — increases mood by 1 level
  const feedBlob = (blobId: string) => {
    let errorMsg = '';
    updateState((prev) => {
      const blob = prev.blobs.find((b) => b.id === blobId);
      if (!blob) {
      ...
      const currentMood = blob.mood?.level ?? 2;
      if (currentMood >= 3) {
        errorMsg = '😊 Already happy!';
        return prev;
      }

      const cost = Math.floor(blob.level * 50);
      if (prev.cubes < cost) {
        errorMsg = `Need ${cost} 💠 to feed`;
        return prev;
      }

      prev.cubes -= cost;
      prev.blobs = prev.blobs.map((b) =>
        b.id === blobId
          ? {
              ...b,
              mood: {
                level: Math.min(3, currentMood + 1) as import('./types').MoodLevel,
                lastFed: Date.now(),
                winsToday: b.mood?.winsToday ?? 0,
                lossesToday: b.mood?.lossesToday ?? 0,
              },
            }
          : b
      );

      triggerToast(`🍖 Fed! Mood: ${{ 0:'😢',1:'😴',2:'😐',3:'😊' }[Math.min(3, currentMood + 1)]}`);
      return prev;
    });
  ...
  // ── Weekly Arena Handlers ──
  const handleRegisterBlobForArena = (blobId: string) => {
    updateState((prev) => {
      prev.arenaRegisteredBlobId = blobId;
      return prev;
    });
    const blob = state.blobs.find(b => b.id === blobId);
    triggerToast(`⚔️ Registered ${blob?.personality.toUpperCase()} Blob for Weekly Arena!`);
  };

  const getArenaRewardCubes = (rank: number): number => {
    if (rank >= 1 && rank <= 10) {
      return ARENA_REWARDS[rank - 1].cubeReward;
    }
    return 250; // participation reward
  };

  const handleClaimArenaReward = () => {
    if (!state.lastArenaRank || state.lastArenaRewardClaimed) return;

    const rewardCubes = getArenaRewardCubes(state.lastArenaRank);

    updateState((prev) => {
      prev.cubes += rewardCubes;
      prev.totalCubesAllTime += rewardCubes;
      prev.lastArenaRewardClaimed = true;
      return prev;
    });

    triggerToast(`🎁 Claimed ${rewardCubes.toLocaleString()} 💠 for placing #${state.lastArenaRank} in the Arena!`);
  };

  const handleDevTriggerWeeklyReset = () => {
    let achievedRank: number | null = null;
    if (state.arenaRegisteredBlobId) {
      const blob = state.blobs.find(b => b.id === state.arenaRegisteredBlobId);
      if (blob) {
        const power = calcBlobPower(blob);
        const rankBase = Math.max(1, 100 - Math.floor(power / 4));
        achievedRank = Math.max(1, Math.min(10, rankBase + Math.floor(Math.random() * 5) - 2));
      }
    } else {
      achievedRank = Math.floor(Math.random() * 10) + 1; // random rank 1-10
    }
    updateState((prev) => {
      prev.lastArenaProcessedWeek = "W-sim-" + Math.floor(Math.random() * 1000);
      prev.lastArenaRank = achievedRank;
      prev.lastArenaRewardClaimed = false;
      prev.arenaRegisteredBlobId = null;
      return prev;
    });
    triggerToast(`🏆 [DEV CHEAT] Weekly reset simulated! Placed #${achievedRank}! Claim your reward in Quests.`);
  };

  // Check and process Arena Monday transition
  useEffect(() => {
    if (!state.lastArenaProcessedWeek) {
      updateState((prev) => {
        prev.lastArenaProcessedWeek = arena.weekKey;
        return prev;
      });
      return;
    }

    if (state.lastArenaProcessedWeek !== arena.weekKey && !state.lastArenaProcessedWeek.startsWith("W-sim-")) {
      const prevRank = arena.myRank;
      updateState((prev) => {
        prev.lastArenaRank = prevRank;
        prev.lastArenaRewardClaimed = prevRank ? false : true;
        prev.lastArenaProcessedWeek = arena.weekKey;
        prev.arenaRegisteredBlobId = null;
        return prev;
      });
      ...
      if (prevRank) {
        triggerToast(`🏆 Weekly Arena has reset! You placed #${prevRank} last week. Claim your rewards in Quests!`);
      }
    }
  }, [arena.weekKey, state.lastArenaProcessedWeek, arena.myRank]);
  ...
  const isBaseApp = typeof window !== 'undefined' && (
    ...
    // Bypass for testing in AI Studio / development environment
    window.location.hostname === 'localhost' ||
    window.location.hostname === '127.0.0.1' ||
    window.location.hostname.endsWith('run.app') ||
    window.location.hostname.includes('googleusercontent.com')
    ...
    <div id="app" className="w-full max-w-[420px] h-[100dvh] max-h-[100dvh] bg-[#060d22] relative flex flex-col shadow-2xl overflow-hidden border-x border-white/5 font-sans mx-auto text-white select-none">
        ...
        {currentScreen === 'home' && (
          <div className="flex flex-col flex-1 animate-fade-in">
            ...
            <div className="relative flex flex-col items-center pt-6 pb-4 flex-shrink-0 min-h-[290px]">
              ...
              {/* Floating action buttons — on top of Home background */}
              <div className="absolute top-2 right-2 z-20 flex flex-col gap-2">
                {/* Arena */}
                <button
                  onClick={() => setShowArenaModal(true)}
                  className="w-10 h-10 rounded-xl bg-purple-900/70 border border-purple-500/40 backdrop-blur-sm flex items-center justify-center hover:bg-purple-800/80 active:scale-95 transition-all shadow-lg"
                  title="Weekly Arena"
                >
                ...
                <button
                  ...
                  className="w-10 h-10 rounded-xl bg-blue-900/70 border border-blue-500/40 backdrop-blur-sm flex items-center justify-center hover:bg-blue-800/80 active:scale-95 transition-all shadow-lg"
                  title="Leaderboard"
                >
                  <span className="text-lg">🏆</span>
                </button>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-slate-400 text-[9px] mt-0.5 flex items-center gap-1">
                        ...
                        <span className="text-[10px]" style={{ color: ['#ef4444','#f59e0b','#94a3b8','#22c55e'][blob.mood?.level ?? 2] }}>
                          {['😢','😴','😐','😊'][blob.mood?.level ?? 2]}
                        </span>
            ...
            {currentSelectedBlob && (() => {
              ...
              return (
                <div className="mx-4 mt-2 mb-6 bg-black/40 border border-white/5 rounded-2xl p-3.5 flex flex-col gap-3 backdrop-blur-md">
                  ...
                  {/* Feed button */}
                  {(() => {
                    const blob = currentSelectedBlob;
                    const mood = blob.mood?.level ?? 2;
                    const cost = Math.floor(blob.level * 50);
                    return (
                      <button
                        onClick={() => feedBlob(blob.id)}
                        disabled={mood >= 3}
                        className="mt-2 w-full py-2 rounded-xl text-xs font-semibold border transition-all active:scale-95 disabled:opacity-40"
                        style={{
                          borderColor: mood >= 3 ? '#22c55e44' : '#ffffff22',
                          color: mood >= 3 ? '#22c55e' : '#94a3b8',
                          backgroundColor: mood >= 3 ? '#22c55e12' : '#ffffff08',
                        }}
                      >
                        {mood >= 3 ? '😊 Happy!' : `🍖 Feed (${cost} 💠)`}
                      </button>
                    );
                  })()}
                </div>
      ...
      {showProfileModal && (
        <ProfileModal
          ...
          leaderboardRank={leaderboard.myRank}
          leaderboardScore={leaderboard.myScore}
      ...
      {showArenaModal && (
        <ArenaModal
          ...
          myRank={arena.myRank}
          myEntry={arena.myEntry}
          weekKey={arena.weekKey}
          msUntilEnd={arena.msUntilEnd}
