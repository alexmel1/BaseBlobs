...
export const XP4LV = (lv) => lv * 60;
export const EREGEN = 5 * 60 * 1000;
export function getBlobStats(personality, level) {
    const base = {
 ...
 */
export const DEFAULT_BRANCHES = ['speed', 'harvest', 'fortune'];
...
export function getBlobBranches(blob) {
...
export function getUpgradeSlots(level) {
    if (level >= 10)
        return 3;
    if (level >= 5)
        return 2;
...
export function canUpgrade(branchId, currentLevel, // current upgrade level (0 = not purchased)
blobLevel, blobUpgrades, cubes, evolutionStage, 
...
availableBranches = DEFAULT_BRANCHES) {
    ...
    if (!branch)
        return { allowed: false, reason: 'Unknown branch' };
 ...
 */
export function getUpgradeValue(id, level, fallback) {
    const lvl = level ?? 0;
    if (lvl <= 0)
        return fallback;
...
export function applyUpgrades(baseReward, baseDuration, baseBonusChance, upgrades) {
...
export const MOOD_CONFIG = {
    ...
    feedCost: (level) => Math.floor(level * 50),
    // Emoji per level
    emoji: { 0: '😢', 1: '😴', 2: '😐', 3: '😊' },
    // Names
    names: { 0: 'Sad', 1: 'Tired', 2: 'Neutral', 3: 'Happy' },
};
...
export const NODE_CONFIG = {
    ...
    cubesPerHour: { 1: 10, 2: 25, 3: 60, 4: 150, 5: 500 },
    // Required blob level for attacking
    minBlobLevel: { 1: 1, 2: 1, 3: 2, 4: 3, 5: 4 },
    // Tier colors
    tierColors: {
        1: '#0088ff',
        2: '#6600ff',
...
export const DEFAULT_BLOB_MOOD = {
...
export function calcDefensePower(blob) {
 ...
 */
export function canAttackNode(blobPower, node) {
    if (!node.owner && !node.isNPC)
        return true; // empty node
    const defenderPower = node.isNPC ? node.npcPower : node.blobPower;
    const effectiveDefense = defenderPower * (1 + (node.fortifyBonus || 0) / 100) * (node.guardMult || 1);
 ...
 */
export function buildArenaFighter(blob) {
 ...
 */
function resolveDuel(duel, player, enemy, rng, log) {
...
export function resolveArenaMatch(playerSquad, enemySquad, rng = Math.random) {
    const duels = [];
    const log = [];
    let playerWins = 0;
...
export function calcMmrDelta(opts) {
    ...
    if (isBot)
        k *= ARENA_CONFIG.botMmrFactor;
    return Math.round(k * (actual - expected));
}
export function arenaScoreLabel(playerWins) {
    return `${playerWins}:${ARENA_CONFIG.squadSize - playerWins}`;
}
export function arenaRewardFor(playerWins) {
